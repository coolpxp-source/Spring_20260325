...
public class DefaultService {
	...
	// ex) 학생 목록 : getStudentList, 학생 수정 :  editStudent
	
	// ====== Mapper 호출 시 ===== 
	// 여러개 리턴 -> selectXXXList
	//	List<User> list = defaultMapper.selectUserList();
	// 한 개 리턴 ->  selectXXX
	// User info = defaulMapper.selecUser();
	// 수정, 삭제, 삽입 -> updateXXX, deleteXXX, insertXXX
	// int result = defaulMapper.insertXXX();
	public HashMap<String, Object> 함수이름(HashMap<String, Object> map){
		HashMap<String, Object> resultMap = new HashMap<String, Object>();
		
		try {
			//	List<User> list = defaultMapper.selectUserList(map);
			// User info = defaulMapper.selecUser(map);
			// int result = defaulMapper.insertXXX(map);
